...
int leftKeyReading[5];
int lastLeftKeyReading[5];
...
int rightKeysThreshold[5] = {125, 130, 130, 135, 145};
int rightKeyVal[5];
int lastRightKeyVal[5];
int rightKeyReading[5];
int lastRightKeyReading[5];
...
void setup() {
  ...
  pinMode(lOctBtn1, INPUT_PULLUP);
  pinMode(lOctBtn2, INPUT_PULLUP);
  pinMode(rOctBtn1, INPUT_PULLUP);
  pinMode(rOctBtn2, INPUT_PULLUP);
//  Serial.begin(9600);
...
//    usbMIDI.sendControlChange(110, lipOut, 3);
  ...
  lastLeftOctave = leftOctave;
  int tempOctave1 = !digitalRead(lOctBtn1);  
  int tempOctave2 = !digitalRead(lOctBtn2);  

  if(tempOctave1 == 1 && tempOctave2 == 0){
    leftOctave = -12;
  }
  else if(tempOctave1 == 0 && tempOctave2 == 1){
    leftOctave = 12;
  }
  else if(tempOctave1 == 0 && tempOctave2 == 0){
    leftOctave = 0;
  }
  else if(tempOctave1 == 1 && tempOctave2 == 1){
    leftOctave = -24;
  }
  ...
  if(breath != 0){
    ...
    if(leftOctave != lastLeftOctave){
      for(i = 0; i < 5; i++){
        if(leftKeyVal[i] == 1){
          currentNote = notes[i] + lastLeftOctave;
    ...
    for(i = 0; i < 5; i++){
      ...
      lastLeftKeyReading[i] = leftKeyReading[i];
      leftKeyReading[i] = touchRead(leftKeys[i]);
      leftKeyReading[i] = expFilter(leftKeyReading[i], lastLeftKeyReading[i], 1024, 0.005);
      if(leftKeyReading[i] < leftKeysThreshold[i]){
        leftKeyVal[i] = 0;
      }
      else{
        leftKeyVal[i] = 1;
  ...
  lastRightOctave = rightOctave;
  tempOctave1 = !digitalRead(rOctBtn1);  
  tempOctave2 = !digitalRead(rOctBtn2);  
  ...
  if(breath != 0){
    ...
    if(rightOctave != lastRightOctave){
      for(i = 0; i < 5; i++){
        if(rightKeyVal[i] == 1){
          currentNote = notes[i] + lastRightOctave;
    ...
    for(i = 0; i < 5; i++){
      ...
      lastRightKeyReading[i] = rightKeyReading[i];
      rightKeyReading[i] = touchRead(rightKeys[i]);
      rightKeyReading[i] = expFilter(rightKeyReading[i], lastRightKeyReading[i], 1024, 0.005);
